...
# def cap_constr(model,i):
#     if i == 0:
#         return sum(model.x[i,j] for j in rates) <= c[i] + sum(returns[i,j] for j in rates)    
#     elif 1<=i<=3:
#         return sum(model.x[i,j] for j in rates) <= model.c[i] - sum(model.x[i-1,j] for j in rates) + model.x[i-1,0] +sum(returns[i,j] for j in rates if j >=1)  
#     elif 4<=i<=7:
#         return sum(model.x[i,j] for j in rates) <= model.c[i] - sum(model.x[i-1,j] for j in rates) + sum(model.x[i-1,j] for j in rates if j <=1) + sum(returns[i,j] for j in rates if j >=2)
#     elif 8<=i<=15:
#         return sum(model.x[i,j] for j in rates) <= model.c[i] - sum(model.x[i-1,j] for j in rates) + sum(model.x[i-1,j] for j in rates if j <=2) + sum(returns[i,j] for j in rates if j >=3)
#     elif i >= 16:
#         return sum(model.x[i,j] for j in rates) <= model.c[i] - sum(model.x[i-1,j] for j in rates) + sum(model.x[i-1,j] for j in rates if j==0) + sum(model.x[i-4,j] for j in rates if j==1) + sum(model.x[i-8,j] for j in rates if j==2) + sum(model.x[i-16,j] for j in rates if j==3)
# model.cap=Constraint(weeks, rule=cap_constr)


# def cap_constr1(model,i):
#     if i == 0:
#         return sum(model.x[i,j] for j in rates) <= model.c[i] + sum(returns[i,j] for j in rates)    
#     else:
#         return Constraint.Skip
# model.cap1=Constraint(weeks, rule=cap_constr1)

# def cap_constr2(model,i):
#     if i == 0:
#         return Constraint.Skip
#     elif 1<=i<=3:
#         return sum(model.x[i,j] for j in rates) <= model.c[i] - sum(model.x[i-1,j] for j in rates) + model.x[i-1,0] + sum(returns[i,j] for j in rates if j >=1)  
#     else:
#         return Constraint.Skip
# model.cap2=Constraint(weeks, rule=cap_constr2)

# def cap_constr3(model,i):
#     if i <= 3:
#         return Constraint.Skip
#     elif 4<=i<=7:
#         return sum(model.x[i,j] for j in rates) <= model.c[i] - sum(model.x[i-1,j] for j in rates) + sum(model.x[i-1,j] for j in rates if j <=1) + sum(returns[i,j] for j in rates if j >=2)
#     else:
#         return Constraint.Skip
# model.cap3=Constraint(weeks, rule=cap_constr3)   

# def cap_constr4(model,i):
#     if i <= 7:
#         return Constraint.Skip
#     elif 8<=i<=15:
#         return sum(model.x[i,j] for j in rates) <= model.c[i] - sum(model.x[i-1,j] for j in rates) + sum(model.x[i-1,j] for j in rates if j <=2) + sum(returns[i,j] for j in rates if j >=3)
#     else:
#         return Constraint.Skip
# model.cap4=Constraint(weeks, rule=cap_constr4)

# def cap_constr5(model,i):
#     if i <= 15:
#         return Constraint.Skip
#     elif i >= 16:
#         return sum(model.x[i,j] for j in rates) <= model.c[i] - sum(model.x[i-1,j] for j in rates) + sum(model.x[i-1,j] for j in rates if j==0) + sum(model.x[i-4,j] for j in rates if j==1) + sum(model.x[i-8,j] for j in rates if j==2) + sum(model.x[i-16,j] for j in rates if j==3)
#     else:
#         return Constraint.Skip
# model.cap5=Constraint(weeks, rule=cap_constr5)
